...
int CHudScope::MsgFunc_Scope(const char* pszName, int iSize, void* pbuf)
{
	...
	float fovScope = 20.0f;
	float fovDefault = 0.0;

	if (fOn)
	{
		m_iFlags |= HUD_ACTIVE;
		inter.Interpolate(0.0f, vec, vec, &fovScope);
	}
	else
	{
		m_iFlags &= ~HUD_ACTIVE;
		inter.Interpolate(0.0f, vec, vec, &fovDefault);
	}
...
bool CHudScope::Draw(float flTime)
{
	return true;
}
